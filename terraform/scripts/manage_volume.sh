...
if [ $# -eq 0 ]; then
    echo "Usage:"
    echo "./manage_volume.sh -o create -r lon1 -s 15 -n mybooks"
    echo
    ...
    echo "  -n : the name of your volume (default mybooks)"
    echo
    exit 1
fi
...
then
    ...
    then
        ...
        -d "{\"size_gigabytes\":15, \"name\": \"${name}\", \"description\": \"Persistent volume for mybooks project\", \"region\": \"${region}\", \"filesystem_type\": \"ext4\"}" \
