const autoCompleteJS = new autoComplete({
    ...
    data: {                              // Data src [Array, Function, Async] | (REQUIRED)
        src: async () => {
            ...
            const source = await fetch(`/books/search?q=${query}`);
    ...
    resultItem: {
        ...
        content: (data, element) => {
            ...
            element.setAttribute("href", "/book/" + data.value.ID)
...
function generateBookItem(data, title) {
    ...
    return  `<div  class="block col-span-1"><img class="w-full h-auto" src="${data.Thumbnail}"/></div>
